...
| `agents` | `agents[]` sorted by spend (id, type, description, model, state — the notification's word where one arrived — `status` (completed / failed / killed, or null), elapsed, tokens, `cost` (USD, `approx`, `source`), `returned_tokens` (the result's length ÷ 4, or null), `waste` (`{usd, reason: failed | killed | no_ret | idle, idle_ms}` or null), `cold_start`, `launched_turn`, `workflow`, `inherited_context` (a fork's first own call's cache read, in tokens), `fork_context_messages` (Claude Code's `contextLength`, not tokens), depth), `totals` (cost, waste, waste_by_reason, classified, cold_starts, cold_start_usd, returned_ratio), `workflows[]` (run, launched, done, failed, empty_result, agents, cost, waste), `mcp[]` (name, pid, rss, restarts, calls) or missing for fixtures, `tasks[]`, `teammates[]` |
