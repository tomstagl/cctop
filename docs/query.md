...
| `dashboard` | Console, schema 2 (`schema` was 1 before 0.6.0: `tiles`, `nudge` and `rows[9]` are gone, replaced by what follows; the pane reads `schema` and says which cctop it needs on a mismatch): `header` (session, model, version, turn, elapsed, cwd, pr, `team` — `<member>@<team>` only when the attached session is itself a teammate — phase {glyph, word, elapsed, tokens}, line), `cells[6]` (key `1`–`6`, id, opens, and three forms of the label as lines of `{text, tone}` segments: `label` for ≥ 80 body columns, `mid` for cells ≥ 28 wide, `short` below), `act` (key `a`, opens, line, short, tag, nudge or absent, blocked, acting — the coach's slot: a wait pre-empts the nudge, the nudge the state line's tokens, those the first turn's start line), `bodies[8]` (key `1`–`6`, `a`, `0`; id context / limits / cache / cost / work / tools / advisor / events; title; keys; `rows[]` of segment lines, full length — each surface cuts once at its own width; `slices[]` {label, tokens, step} for the two bodies with a bar, step ∈ 0/1/2 into the theme's series ramp, never a colour), tone ∈ fg/dim/accent/ok/warn/crit/bold/s0/s1/s2, `session_mode`, `lines` (the coach's L0/L1/L2, the inline strip), `cost_combined` (as in `summary`). The object does not know which body a surface has open: all eight are in it |
| `prefix` | `total` and `rows[]` (kind, name, bytes, tokens, count) of what rides on every request |
| `sources` | What is in the window right now and what put it there: `size`, `window`, `prefix` (with `prefix_share`, `prefix_tightened`, `mode` — `estimated`, or `calibrated_at_turn` from a `/context`), `since` (`session-start`, or the last boundary's `kind`, `call` and `delta`), `calls_since`, `rows[]` (source, tokens, share — thinking and prose exact, the rest ≈), `files[]` (path, read, written, reads), `reconciled` and `overflow_raw` (what the estimates would have exceeded the size by), `model_switch_kept`. The `m` inspector's object |
