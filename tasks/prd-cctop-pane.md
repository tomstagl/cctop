...
**Status:** v1.3 · 2026-09-15 — shipped (plugin 0.4.1 with cctop 0.3.0; the v1.3 note at the end of §11 records the clock contract of Claude Code ≥ 2.1.271, issue #3); §11 amends the view set, the poller and the engine surfaces for the coach (v1 reviewed against the generated `plugin/.claude/types/claude-code.d.ts` of 2.1.269 and the binary; corrections marked *v1.1*; the coach amendment marked *v1.2*)
...
**v1.3 — the clock contract (2026-09-15, issue #3).** Claude Code 2.1.271 turned `$.clock.now()` from a synchronous number into a host event that resolves a Promise (`clock.now`; `sleep`, `after` and `every` are host events too, `every` at least every 1 ms). Plugin 0.4.0, written against 2.1.270, did arithmetic on the value and failed every hook on 2.1.271 and 2.1.272. From plugin 0.4.1 the module awaits every reading: the turn and tool hooks read the clock once before their `next(e)` (one host round trip, the cost of the hook's own dispatch again — not a `$.process.run`, so FR-10 stands), `ui.render` reads it once per frame for the elapsed times, and `requestRender` — under every model change — folds the changes that land while it reads the clock into one request, so the throttle of US-008 (at most four invalidates a second plus one trailing) holds; the trailing timer needs no second reading. Awaiting a number is fine, so one build runs on 2.1.270 and on ≥ 2.1.271. **Version pin:** the checked-in contract `plugin/.claude/types/claude-code.d.ts` is regenerated against the version named in its first line (2.1.272), `TESTED_WITH` in `model.ts` repeats it and the header badge shows it (`hooks 2.1.272`), and `make check-types` fails when the installed `claude` differs — the guard that should have caught 2.1.271 had it been run. The verification item is 25 of `docs/verification/pane.md`.
