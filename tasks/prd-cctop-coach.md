...
| A11 Fresh-input spike | Use the turn's first API call `input_tokens` or history.jsonl `+N lines`; never the turn sum. *As shipped (2026-09-15):* on a cached session the paste is not `input_tokens` at all — Claude Code writes the new prompt into the cache, so it lands in the first call's `cache_creation_input_tokens` with the previous turn's tail (checked on 21 local pastes: `input_tokens` 2–10, `cache_creation` 6.9 k for a 134-line paste); the size therefore comes from history.jsonl — inline characters (≥ 20 k) or the `+N lines` placeholder of a hashed paste (≥ 200 lines, ≈ 25 tokens a line), matched to the turn by time — and `input_tokens > 5 k` stays only for uncached sessions |
...
- [x] history.jsonl watcher (US-007) also feeds `/clear` and `/compact` frequency as habit baselines and the `pastedContents` size for A11 — *as shipped:* `History.pastes` keeps `(at, chars, lines)` per session — characters from inline `content`, lines from the `[Pasted text #k +N lines]` placeholders (the hashed pastes' text in `paste-cache/` is pruned) — and A11 matches them to turns by time; `cctop coach-replay` folds the machine's history rows in by session as the clock passes them
