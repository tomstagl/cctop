...
| D2c | `projects/<slug of member.cwd>/<sessionId>.jsonl` — a teammate's transcript, matched by `teamName == session-<lead id8>` on its lines | `~/.claude/teams` watch + head scan + tail | **A teammate of an agent team is a full session with its own transcript and its own `cost-state`** (`docs/teams.md`, `tasks/prd-cctop-team-costs.md`). Every `user` / `assistant` / `system` / `attachment` line carries `agentName` and `teamName`; the team directory (D12) is removed when the team ends, the transcripts stay. Same parser, same dedupe rule, same ledger arithmetic as D2 + D11. |
...
| D12 | `~/.claude.json`, `settings.json`, the `claude` process environment, `~/.claude/teams/<team>/config.json` | read on attach; the team directory watched | Claude Code's own configuration: the plan tier, autocompact overrides, the team's membership and each `tmux` member's `isActive` (no session id: the transcript is matched by `teamName` / `agentName`) |
