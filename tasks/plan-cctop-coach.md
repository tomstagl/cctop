...
**For:** `tasks/prd-cctop-coach.md` v1.2 (13 user stories, 34 rules, the "Lights" view)
...
**Written:** 2026-09-14 · **Amended** the same day: the dashboard itself is direction B (`tasks/plan-dashboard-big-figures.md`) — the framed nine-panel grid is replaced by four block-digit tiles, the nudge and a nine-row ledger; Phase 2 and Phase 4 below are read with that plan. **Aligned** with PRD v1.2 the same evening (both documents now say the same thing about the dashboard; the plan gained the acceptance criteria it had dropped — A04's counting rewrite, the 200-char command summary, image tokens, the `cache` object of `query summary`, the header additions, the cold-price switch, the coach's Events rows and SessionEnd tally, `tipsHistory` suppression, and where US-013 actually lands).
...
**Progress.** Phase 0 shipped 2026-09-14 (`4547948`). Phase 1 shipped 2026-09-14 in seven commits (`8692b3a` transcript fields + fixture B, `1f9310b` turn identity and the number changes, `0691529` phase.rs, `f3c281b` status and the cache clock, `847463b` hooks, `4252e00` files/A04, plus the fixture-B query snapshots): every field the PRD names is parsed, `cctop query summary` / `ledger` / `events` carry the new facts, both fixtures and both surfaces' tests are regenerated; the phase classifier agrees with the research reference on 99.9 % of 5 163 real commands. Phase 2 shipped 2026-09-15 in five commits (`38d317c` Context, `d7a4434` Tokens & Cost, `9984ae2` Turn and Limits, `8725ec3` Tools / Agents / Files / Prefix / Ledger / alerts, plus the header and the fixture-B snapshots): every §4 row exists as a metric with a registry row, the full-screen panels draw them, `cctop query` carries them, and the alert table is US-013's. Left for later phases by design: the personal 5-hour-block ceiling for API-key users (baseline.rs, Phase 7), the pane's B fixtures and the numeric `/context` agreement test on a live session (Phase 4, with the dashboard object — fixture B's `/context` capture comes from another session than its `prompt_snapshot`, so only the structural agreement is tested).

| Phase | Stories | Ships | Size |
|---|---|---|---|
| 0 | — | nav bar inside the pane, frame digits = TUI panel ids, the design canvas | done |
| 1 | US-001, US-002, US-003, US-013 (the number changes) | every field the PRD names is parsed; fixture B; `promptId` turns; `phase.rs`; `harness_facts.rs`; the user-visible number changes land here with regenerated fixtures | L |
| 2 | US-004, US-005, US-013 (the alert table) | panels agree with `/context`, `/usage`, the footer; cost gradient; agents found; error lines never a compaction — the rows land in the ledger and the full-screen panels of plan B, not in framed blocks | L |
| 3 | US-006, US-013 (one dismissal model, `query advice` schema) | engine v2: classes, TTLs, cooldowns, acted, persistence, session_mode; A01–A18 recalibrated | M |
| 4 | US-009, US-010, US-013 (the pane PRD amendments) | `coach::snapshot` → `cctop query coach` → TUI `c` view, pane Coach view, `$.ui.status`, MCP tool, skill; **and the B dashboard** (`dashboard::snapshot` → `cctop query dashboard` → TUI dashboard + pane Overview, plan B §5 steps 1–4) | L |
...
- `src/tools.rs`: Bash `input_summary` widens to a bounded 200-char command; image results count `w·h/750` tokens (1 500 unknown) from `toolUseResult.file.dimensions`; `persisted_output_size` sits beside `result_tokens_est`; `Call.phase` from `phase.rs`. `src/files.rs`: the A04 counting rewrite of US-001 — Bash `cat`/`sed -n`/`head`/`tail` count as reads, ranged reads and `file_unchanged` results are excluded, the re-read counter resets on `edited_text_file`, `staleRecovered` and every boundary (`/clear`, `continued-in`, resume, compact, fork).
...
- `src/status.rs::Sample`: `prompt_cache.*` (14 keys), `effort.level`, `thinking.enabled`, `fast_mode`, `exceeds_200k_tokens`, `context_window.*`, `rate_limits.spend_limit`, `session_name`, `prompt_id`, `version`, `pr.*`, `worktree.*`; drop the never-populated `plan` (the header's tier comes from `~/.claude.json .oauthAccount.userRateLimitTier`, keys only). `State` exposes `cache_expires_at_ms`, `cache_ttl_source`, `recache_tokens_if_cold`, `misses`, `expected_rebuilds`, `last_miss_cause`; the countdown is clock-driven between rewrites; the shim-less fallback is marked `≈`; `cctop query summary` gains a `cache` object with metric ids.
- `src/hooks.rs::apply_hook`: `duration_ms` (drop `≈`), `is_interrupt`, `error`, `Stop.{background_tasks, session_crons}` plus the shape of `last_assistant_message` (length, ends with `?`), `SessionStart.*` resume fields, `Notification.notification_type`, `permission_mode`, `effort.level`, `prompt_id`, `agent_id/agent_type` routed to the agent's stats. `cctop hook` keeps `permission_suggestions` and Agent `tool_response`, and reduces `UserPromptSubmit.prompt` / `Stop.last_assistant_message` to their shape before spooling (FR-12). `src/install.rs` registers the ten missing events observe-only, shows the diff, uninstall restores. `src/tasks.rs` stops listing dotfiles; background tasks come from `Stop.background_tasks`.
...
- Header (`src/ui/panels/header.rs`, plan B's `dashboard.header`): session title (`ai-title`/`custom-title`), PR number (`pr-link`), effective effort (`perTurnEffort` › `effort` › status `effort.level`) with thinking on/off and fast mode, permission mode incl. `plan`, "prev. session here: $X, N min" from `~/.claude.json projects[cwd].last*`.
...
Dogfood starts here: open the coach on both surfaces for a working week with A01–A18 only; log every `x` (US-012 counts them). US-009's "every P0 rule fired once correctly" criterion closes after Phase 6, when the P0 rules exist.
