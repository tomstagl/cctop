...
pub const METRICS: &[Metric] = &[
    ...
    metric!(coach_rework, "Coach", "Rework light", "state", "Open issues: consecutive failed calls of the turn (denials excluded), corrections (interrupts, rejected calls) in the last three turns, blocked calls; else the source edits since the last confirmed test run. The figure is a state word — `N open`, `N unchecked`, `ok`, or `—` before the session has called anything — never a `0` that means healthy and no data alike. ◐ after 10 min or 14 calls unverified, two fails, a PR without a review, an uncommitted tail; ● on a cascade, a denial streak, a correction streak, destructive git on a dirty tree, a commit without a check", ["D1", "D7"], "", ""),
];
